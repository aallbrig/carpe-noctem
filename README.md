...
```
apm list --installed --bare > provisioning/roles/dev-machine/files/atom-requirements.txt
```
...
### Common commands
```
ansible-playbook provisioning/setup-workstation.yml --ask-sudo-pass --extra-vars "@config.yml" --tags "vagrant"
ansible-playbook provisioning/spinup-env.yml --extra-vars "@config.yml" --tags "jenkins"
```
```
...
You may encounter an issue related to the hidden directory `.vagrant` being generated by a sudo user.  Run `sudo rm -rf .vagrant` to delete, then `ansible-playbook provisioning/setup-workstation.yml --ask-sudo-pass --extra-vars "@config.yml" --tags "vagrant"` to generate a new `.vagrat` directory and `Vagrantfile`.
